...
static nrf_pwm_values_wave_form_t m_idteck_pwm_seq_vals[IDTECK_RAW_SIZE * IDTECK_PSK_CYCLES_PER_BIT * IDTECK_PSK_ENTRIES_PER_CYCLE] = {};
...
static const nrf_pwm_sequence_t *idteck_modulator(idteck_codec *d, uint8_t *buf) {
    ...
    for (int i = 0; i < IDTECK_RAW_SIZE; i++) {
        ...
        for (int j = 0; j < IDTECK_PSK_CYCLES_PER_BIT; j++) {
            m_idteck_pwm_seq_vals[k].channel_0 = first;
            m_idteck_pwm_seq_vals[k].counter_top = IDTECK_PSK_COUNTER_TOP;
            k++;
            m_idteck_pwm_seq_vals[k].channel_0 = second;
            m_idteck_pwm_seq_vals[k].counter_top = IDTECK_PSK_COUNTER_TOP;
            k++;
        }
    }

    m_idteck_pwm_seq.length = k * 4;
